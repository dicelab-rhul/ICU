...
def start_ui(event_system):
    ...
    ui_process = start(
        ui_remote_source, ui_remote_sink
    )  # TODO we dont want to provide the full default configuration!?
...
async def main(cmdargs):
    # TODO create an event that is triggered when the event system is created? This is the "start time".
    ...
    # TODO an event that is triggered when pygame/the UI is fully initialised?
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--logpath",
        help="Location for the event log file. This may be overriden by the --logfile option.",
        default=DEFAULT_LOGPATH,
    )
    parser.add_argument(
        "--log",
        help="Path to the event log file. If specified this will override the --logpath option.",
        default=None,
    )
    parser.add_argument(
