...
DEFAULT_WINDOW_CONFIG = dict(
    size=Point(WindowInfo.screen_size),
    position=Point(0, 0),
    full=False,  # TODO doesnt do anything yet, there are problems on linux and fullscreening here :/
    resizable=False,  # TODO doesnt do anything yet.
...
class Window(Widget):
    ...
    def __init__(self):
        ...
        self._window, self._flags = new_pygame_window(
            self.position.get(),
            self.size.get(),
        ...
        self._event_gen = {
            ...
            pygame.MOUSEMOTION: lambda event: self.source(
                UI_INPUT_MOUSEMOTION, dict(x=event.pos[0], y=event.pos[1])
            ),
            pygame.MOUSEBUTTONDOWN: lambda event: self.source(
                UI_INPUT_MOUSEDOWN,
                dict(x=event.pos[0], y=event.pos[1], button=event.button),
            ),
            pygame.MOUSEBUTTONUP: lambda event: self.source(
                UI_INPUT_MOUSEUP,
                dict(x=event.pos[0], y=event.pos[1], button=event.button),
            ),
            pygame.KEYDOWN: lambda event: self.source(
                UI_INPUT_KEYDOWN,
                dict(keycode=event.key, mod=event.mod, unicode=event.unicode),
            ),
            pygame.KEYUP: lambda event: self.source(
                UI_INPUT_KEYUP,
                dict(keycode=event.key, mod=event.mod, unicode=event.unicode),
            ),
        }
    ...
    def size(self, value):
        ...
        self.source(
            UI_WINDOW_WINDOWRESIZED, dict(width=self._size.x, height=self._size.y)
        )
    ...
    def _source_window_reposition(self, event):
        if int(self.position[0]) != int(event.x) or int(self.position[1]) != int(
            event.y
        ):
            ...
            self.source(
                UI_WINDOW_WINDOWMOVED, dict(x=self._position.x, y=self._position.y)
            )
    ...
    def _source_window_focus(self, event):
        if event.type == pygame.WINDOWFOCUSGAINED:
            ...
            raise ValueError(
                f"Invalid event {event} encountered when setting window focus."
            )


def new_pygame_window(
    window_position,
    window_size,
    title=WINDOW_TITLE,
    resizeable=True,
    ...
    flags = 0
    if resizeable:
        flags |= pygame.RESIZABLE  # pylint: disable=E1101
    if fullscreen:
        flags |= pygame.FULLSCREEN  # pylint: disable=E1101
...
def get_pygame_window_info():
    npw = (
        pwc.getActiveWindow()
    )  # TODO must check that the active window is actually the one we are interested in! (ICU)
    return SimpleNamespace(
        window_size=(npw.width, npw.height),
        window_position=(npw.left, npw.top),
        window_title=npw.title,
    )
