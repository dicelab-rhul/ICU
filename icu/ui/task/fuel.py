...
class FuelTank(Widget):
    ...
    def draw(self, window):
        ...
        draw_rectangle(
            window, position=position, size=size, color=self._fuel_color, fill=True
        )
        draw_rectangle(
            window,
            position=self.canvas_position,
            size=self.size,
            line_width=self.parent.line_width,
...
@cosmetic_options(fuel_level_protrusion=FUELTANKMAIN_FUEL_LEVEL_PROTRUSION)
class FuelTankMain(FuelTank):
    ...
    def draw(self, window):
        ...
        draw_line(
            window,
            start_position=(px - ix, cy),
            end_position=(px + self.size[0] + ix - 1, cy),
            width=int(iy),
    ...
    def _get_pump_position(self, tank2, pump):
        ...
        x = self.position[0] + (
            self.size[0] * (1 - w)
        )  # either side of the correct tank
...
class FuelTankLeft(FuelTank):
    ...
    def _get_pump_position(self, tank2, pump):
        if isinstance(tank2, FuelTankMain):
            ...
            y = (
                self.position[1]
                - (self.position[1] - (tank2.position[1] + tank2.size[1])) / 2
            )
            return Point(x, y) - pump.size / 2
        else:
            raise ValueError(
                f"Invalid tank {type(self).__name__} for {type(self).__name__}, tanks do not share a {Pump.__name__}."
            )  # unreachable...
...
class FuelTankRight(FuelTank):
    ...
    def position(self):
        ps, s = self.parent.size, self.size
        return Point(
            ps[0] - s[0] - self.parent.padding[0], ps[1] - s[1] - self.parent.padding[1]
        )
    ...
    def _get_pump_position(self, tank2, pump):
        if isinstance(tank2, FuelTankMain):
            ...
            x = (tank2.position[0] + tank2.size[0] / 2) + (
                self.position[0] - tank2.position[0]
            ) / 2
...
class Pump(Widget):
    ...
    def state(self, value):
        if isinstance(value, int):
            ...
            raise ValueError(
                f"Failed to set `state`, value {value} must of type {PumpState} or int."
            )
    ...
    def draw(self, window):
        ...
        draw_rectangle(
            ...
            position=self.canvas_position,
            size=self.size,
            color=self._colors[self.state.value],
            fill=True,
        )
        draw_rectangle(
            window,
        ...
        draw_arrow(
            window,
            start_position=self.canvas_position + self.size / 2,
            length=self.size[0] / 4,
            color=self.arrow_color,
            angle=angle,
            head_only=True,
            fill_head=self.arrow_fill_head,
            head_length=self.size[0] / 2,
        )
        super().draw(window)
...
class FuelTask(Widget):
    def __init__(self, event_frequency=30):
        ...
        # add tanks
        self._wings[0].add_child(
            FuelTankMain(
                "FUELTANKA",
                FUELTANKMAIN_CAPACITY / 2,
                FUELTANKMAIN_CAPACITY,
                FUELTANKMAIN_FAILURE_INTERVAL,
            )
        )
        self._wings[0].add_child(
            FuelTankLeft("FUELTANKB", FUELTANKLEFT_CAPACITY / 2, FUELTANKLEFT_CAPACITY)
        )
        self._wings[0].add_child(
            FuelTankRight(
                "FUELTANKC", FUELTANKRIGHT_CAPACITY / 2, FUELTANKRIGHT_CAPACITY
            )
        )
        self._wings[1].add_child(
            FuelTankMain(
                "FUELTANKD",
                FUELTANKMAIN_CAPACITY / 2,
                FUELTANKMAIN_CAPACITY,
        ...
        self._wings[1].add_child(
            FuelTankLeft("FUELTANKE", FUELTANKLEFT_CAPACITY / 2, FUELTANKLEFT_CAPACITY)
        )
        self._wings[1].add_child(
            FuelTankRight(
                "FUELTANKF", FUELTANKRIGHT_CAPACITY / 2, FUELTANKRIGHT_CAPACITY
            )
        )
    ...
    def event_frequency(self, value):
        if value <= 0:
            raise ValueError(
                f"event_frequency must be greater than 0. got {self.event_frequency}"
            )
    ...
    def _draw_wing_connecting_lines(self, window, wing):
        ...
        tl = Point(
            left.canvas_position[0] + left.size[0] / 2,
            main.canvas_position[1] + main.size[1] * 3 / 4,
        )
