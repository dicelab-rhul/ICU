...
@cosmetic_options(
    line_color=TRACKING_LINE_COLOUR, line_width=TRACKING_LINE_WIDTH, scale=1
)
class Target(Widget):
...
class TrackingTask(Widget):
    ...
    @settable_properties(
        "target_speed", "event_frequency", "failure_boundary_proportion"
    )
    def __init__(
        self,
    ...
    def target_speed(self, value):
        if value <= 0:
            raise ValueError(
                f"target_speed must be greater than 0, got {self.target_speed}"
            )
    ...
    def failure_boundary_proportion(self, value):
        if value <= 0 or value >= 1:
            raise ValueError(
                f"failure_boundary_proportion {self.failure_boundary_proportion} not in the interval (0,1) (exclusive)."
            )
    ...
    def user_control_target(self):
        ...
        if smt and self._target_direction.abs().sum():
            ...
            self.target.position = (
                p[0] + self.target_speed * dif * d[0],
                p[1] + self.target_speed * dif * d[1],
            )
    ...
    def draw(self, window):
        ...
        draw_rectangle(
            window, position=pos, size=size, color=self.background_color, fill=True
        )
        # draw lines
        size = size[0]
        ...
        draw_line(
            window,
            start_position=tgp(0, edge),
            end_position=tgp(line_size, edge),
            color=line_color,
            width=line_width,
        )
        draw_line(
            window,
            start_position=tgp(edge, 0),
            end_position=tgp(edge, line_size),
            color=line_color,
            width=line_width,
        )
        draw_line(
            window,
            start_position=tgp(0, size - edge2),
            end_position=tgp(line_size, size - edge2),
            color=line_color,
            width=line_width,
        )
        draw_line(
            window,
            start_position=tgp(edge, size - 1),
            end_position=tgp(edge, size - line_size - 1),
            color=line_color,
            width=line_width,
        )
        draw_line(
            window,
            start_position=tgp(size - edge2, size - edge2),
            end_position=tgp(size - line_size - edge2, size - edge2),
            color=line_color,
            width=line_width,
        )
        draw_line(
            window,
            start_position=tgp(size - edge2, size - 1),
            end_position=tgp(size - edge2, size - line_size - 1),
            color=line_color,
            width=line_width,
        )
        draw_line(
            window,
            start_position=tgp(size - edge2, edge),
            end_position=tgp(size - line_size - edge2, edge),
            color=line_color,
            width=line_width,
        )
        draw_line(
            window,
            start_position=tgp(size - edge2, 0),
            end_position=tgp(size - edge2, line_size),
            color=line_color,
            width=line_width,
        )
