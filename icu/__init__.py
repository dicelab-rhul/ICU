...
def start(sinks=[], sources=[], **kwargs):
    ...
    send, receive = PipedMemory(
        event_sinks=None, event_sources=None, config=None, window_properties=None
    )  # TODO more?
...
def run(shared=None, sinks=[], sources=[], config=None, config_hook=None, logger=None):
    ...
    try:
        ...
        class System(event.EventCallback):
            ...
            def shutdown(
                self, *args, **kwargs
            ):  # WARNING -- GETS CALLED MULTIPLE TIME (sigterm etc)
                # print("SHUTDOWN")

                # subvert the usual scheduling mechanism (otherwise it wont trigger as root is destroyed...)
                if (
                    not event.GLOBAL_EVENT_CALLBACK.is_closed
                ):  # shutdown has already been called...
                    e = event.Event(
                        self.name, "Global", label="system", command="shutdown"
                    )
            ...
            def resize(self):
                raise NotImplementedError(
                    "TODO - resize events"
                )  # TODO move from main_panel.resize?
        ...
        if task.system:
            system_monitor_widget = system_monitor.SystemMonitorWidget(
                main,
                copy.deepcopy(config.__dict__),
                width=constants.SYSTEM_MONITOR_WIDTH,
        ...
        if task.track:
            tracking_widget = tracking.Tracking(
                main, copy.deepcopy(config.__dict__), size=config.screen_height / 2
            )  # scaled anyway
        ...
        if task.fuel:
            ...
            fuel_monitor_widget = fuel_monitor.FuelWidget(
                main,
                copy.deepcopy(config.__dict__),
                width=constants.FUEL_MONITOR_WIDTH,
        ...
        if config.overlay["enable"]:
            if config.overlay["arrow"]:
                print("ARROW ENABLED!")
                # arrow = main.create_oval(-20, -20, 20, 20, fill="red", width=10)
                arrow = main.create_polygon(
                    [-10, -5, 10, -5, 10, -10, 20, 0, 10, 10, 10, 5, -10, 5],
                    fill="red",
                    width=0,
                )
                main.overlay(arrow)
                ...
                class ArrowRotator:
                    def __init__(self):
                        super().__init__()
                        self.highlights = (
                            highlight.Highlight.__all_highlights__
                        )  # hack..
                        self.angle = 0
                        self.highlighted = None
                        event.event_scheduler.schedule(
                            self.rotate_arrow(), sleep=cycle([50])
                        )
                    ...
                    def get_min_distance(self, positions):
                        ...
                        for k, (px, py) in positions.items():
                            d = (
                                (px - self.position[0]) ** 2
                                + (py - self.position[1]) ** 2
                            ) ** 0.5
                            if d < min_d[1]:
                                min_d = (k, d)
                        return min_d
                    ...
                    def rotate_arrow(self):
                        while True:
                            ...
                            positions = {
                                k: self.get_highlight_position(k)
                                for k in highlight.all_highlighted()
                            }
                            ...
                            if len(positions) > 0:
                                ...
                                angle = math.atan2(
                                    hy - self.position[1], hx - self.position[0]
                                ) * (180 / math.pi)
                                ...
                                print(
                                    "ARROW_ROTATE",
                                    self.position,
                                    fuel_monitor_widget.position,
                                    fuel_monitor_widget.size,
                                )
                                yield event.Event(
                                    "arrow_rotator_TEST",
                                    "Overlay:0",
        ...
        if et_config.get("enabled", False):
            eyetracker = None
            filter = eyetracking.filter.TobiiFilter(
                5, 200
        ...
        if shared is not None:
            ...
            shared.window_properties = dict(
                window=dict(
                    position=(root.winfo_rootx(), root.winfo_rooty()),
                    size=(root.winfo_width(), root.winfo_height()),
                )
            )
            if task.track:
                shared.window_properties["track"] = dict(
                    position=tracking_widget.position,
                    size=tuple([min(tracking_widget.size)] * 2),
                )  # TODO fix this in tracking widget...?
            if task.system:
                shared.window_properties["system"] = dict(
                    position=system_monitor_widget.position,
                    size=system_monitor_widget.size,
                    **{
                        k: dict(position=v.position, size=v.size)
                        for k, v in system_monitor_widget.warning_lights.items()
                    },
                    **{
                        k: dict(position=v.position, size=v.size)
                        for k, v in system_monitor_widget.scales.items()
                    }
                )
            if task.fuel:
                shared.window_properties["fuel"] = dict(
                    position=fuel_monitor_widget.position,
                    size=fuel_monitor_widget.size,
                    **{
                        k: dict(position=v.position, size=v.size)
                        for k, v in fuel_monitor_widget.tanks.items()
                    },
                    **{
                        k: dict(position=v.position, size=v.size)
                        for k, v in fuel_monitor_widget.pumps.items()
                    }
                )
            # from pprint import pprint
...
def task_system_monitor(config):
    ...
    for scale in scales:
        ...
        event.event_scheduler.schedule(
            generator.ScaleEventGenerator(scale), sleep=schedule
        )
    ...
    for warning_light in warning_lights:
        ...
        event.event_scheduler.schedule(
            generator.WarningLightEventGenerator(warning_light), sleep=schedule
        )
...
def task_tracking(config):
    ...
    for target in targets:
        ...
        event.event_scheduler.schedule(
            generator.TargetEventGenerator(target, **config.__dict__[target]),
            sleep=schedule,
        )
...
def task_fuel_monitor(config):
    ...
    for pump in pumps:
        ...
        event.event_scheduler.schedule(
            generator.PumpEventGenerator(pump, False), sleep=schedule
        )
...
def debug_highlights():
    ...
    def random_highlight():
        while True:
            yield event.Event(
                "DEBUG_HIGHLIGHT", "Highlight:SystemMonitorWidget", label="highlight"
            )
